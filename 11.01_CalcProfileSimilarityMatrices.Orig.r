...
## extract egonet data
ego.names       <- sort(names(recon_egoedges.list))
...
for (i in 110:1) {
    ...
    ##------------------------------------------------------------------
    tmp.fullFeatureset      <- paste( paste(getwd(),"recon.sim.prof.full",sep="/"), paste0(tmp.id, ".ReconFullProfileSimilarityMatrix.Rdata"), sep="/")
    tmp.reducedFeatureset   <- paste( paste(getwd(),"recon.sim.prof.reduced",sep="/"), paste0(tmp.id, ".ReconReducedProfileSimilarityMatrix.Rdata"), sep="/")
